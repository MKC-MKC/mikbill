...
>
> Например:
> `Subscriptions/other` был тихо перемещён в директорию MikBiLL Api `Subscriptions/All/other`;
> Были добавлены новые методы, без соответствующего уведомления в change log.
> Несмотря на это изменение, API метод `other` по-прежнему принимает запросы для получения подписок...
> Однако, обрати внимание, что теперь для подписки нужно использовать этот метод: setAdditional(id: $id, activate: 0/1);
> Вероятнее всего, метод `other` или ветка `Subscriptions/All` будут удалены в будущем...
>
> Можете посмотреть на официальный список подписок и Middleware в документации:
...
```php
$id = 123; # ID услуги.
$middleware = "wink"; # Название сервиса. Метод `other` в последней версии API более не доступен для установки подписок. Используй setAdditional в примере ниже.
```
